...
from smt_cal_demo.calibration.database.support import session,Calibration,Instrument,Measurement,Certificate
...
certificates = session.query(Certificate).filter(
    Certificate.calibration_id == calibration.id).all()
    
c_last = certificates[-1]
    
n_make_label.make_label(calibration.instrument.id, calibration.instrument.serial_number, c_last.id_m, calibration.start_date)
